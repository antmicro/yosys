// Generated by pmgen.py from ql_dsp.pmg
  ...
  void setup(const vector<Cell*> &cells) {
    ...
    for (auto cell : cells) {
      do {
        ...
        if (!(mul->type.in(id_d_mul, id_b_QL_DSP))) continue;
...
#define subpattern(pattern_name) do { block_subpattern_ql_dsp_ ## pattern_name (recursion+1); if (rollback) goto rollback_label; } while(0)
...
#define subpattern(pattern_name) do { block_subpattern_ql_dsp_ ## pattern_name (recursion+1); if (rollback) goto rollback_label; } while(0)
    if (mul->type != id_b_QL_DSP ||
        // Ensure that register is not already used
        ((param(mul, id_b_ENABLE_DSP).as_int() == 1))) {
      dff = nullptr;
